...
public class file{
    ...
    public static double IDW(double[] num, double[] lbs, double[] lb) {
        ...
        double Cap = 0, Dap = 0, Cbp, Dbp, Ccp, Dcp, Cdp, Ddp;
        double _AP = 0, _BP = 0, _CP = 0, _DP;
        ...
        Cdp = Math.sin(B1) * Math.sin(B) + Math.cos(B1) * Math.cos(B) * Math.cos(L1 - L);
    ...
    public static void main(String[] args)
		...
		*
		*/
		String originalDocument = "1";
		String generateDocument = "1out.txt";
		double bz = 36.06, lz = 120.33;
        ...
        //小维度
        m = (int) ((90 - M + 1) * (360 / 2.5 + 1));
        //大维度
        n = (int) ((90 - N + 1) * (360 / 2.5 + 1));
        int a, b, c, d;
        a = m + p;//C
        b = m + q;//D
        c = n + p;//A
        ...
        for (File f : temfs) {
            ...
            while ((valueString = buff.readLine()) != null) {
                ...
                if (1 <= a - len && a - len <= 12) {
                    C = Double.valueOf(strs[a - len + 1]);
                }
                if (1 <= b - len && b - len <= 12) {
                    D = Double.valueOf(strs[b - len + 1]);
                }
                if (1 <= c - len && c - len <= 12) {
                    A = Double.valueOf(strs[c - len + 1]);
                }
                if (1 <= d - len && d - len <= 12) {
                    B = Double.valueOf(strs[d - len + 1]);
                }
            }
            abcd[0] = A;
            abcd[1] = B;
